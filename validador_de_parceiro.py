...
def mapear_colunas(df, mapeamento):
    ...
    for nome_oficial, alternativas in mapeamento.items():
        for alt in alternativas:
            alt_upper = alt.upper()
            if alt_upper in df.columns: colunas_encontradas[alt_upper] = nome_oficial; break 
...
def validar_parceiros(caminho_arquivo):
    ...
    for index, row in df.iterrows():
        ...
        # [Domínio] ATIVO, CLIENTE, FORNECEDOR
        for col_dom in ['ATIVO', 'CLIENTE', 'FORNECEDOR']:
            if row[col_dom] not in ('S', 'N'): adicionar_erro(col_dom, row[col_dom], "Valor inválido. Esperado 'S' ou 'N'.")
